...
pub fn jsonSchema(ctx: *Schema.Context) !Schema {
    ...
    common.description = "Files and folders to skip, matched with `.gitignore`-style glob patterns. A pattern without a `/` matches at any depth, and a trailing `/` matches directories only.\n\n`zig-out`, `vendor`, and `zig-pkg` are always ignored, as well as hidden folders.";
