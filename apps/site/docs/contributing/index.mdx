...
<Tabs>
  <TabItem value="cargo-binstall" label="cargo-binstall" default>
    ```sh cargo install cargo-binstall cargo binstall just typos-cli ```
  </TabItem>
  <TabItem value="cargo" label="cargo">
    ```sh cargo install just typos-cli ```
  </TabItem>
  <TabItem value="homebrew" label="homebrew">
    ```sh brew install just typos-cli ```
  </TabItem>
</Tabs>
...
Check out the [Creating Rules](./creating-rules.md) guide for how to write new
lint rules. A list of rules we want to implement can be found on the
